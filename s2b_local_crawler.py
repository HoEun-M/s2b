...
{key:'tool',label:'수업도구',keywords:['젠스파크','Genspark','학습지원','이음AI','이음 AI','AI 구독','AI구독','Zoom','줌','Vrew','vrew','브루','제미나이','Gemini','클로드','Claude','GPT','지피티','챗지피티','유튜브','YouTube','캔바','Canva','미리캔버스','젭','ZEP','카훗','Kahoot','북크리에이터','패들렛','Padlet','감마','Gamma','수노','Suno','뤼튼','노션','Notion','띵커벨','퀴즈앤','클래스카드','퀴즈렛','다했니','슬라이도','망고보드','투닝','스픽','Speak','일레븐랩스','ElevenLabs','런웨이','Runway','미드저니','Midjourney']},
{key:'hardware',label:'하드웨어·디바이스',keywords:['태블릿','태블릿PC','태블릿 PC','태블릿컴퓨터','태블릿 컴퓨터','노트북','전자칠판','충전함','실물화상기','실물 화상기','크롬북','아이패드','거치대','갤럭시탭','갤럭시 탭','S10FE','S11','키보드','마우스','카메라','웹캠','모니터','스마트기기','디바이스']},
...
function classifyPurchaseType(name,recordId){if(recordId&&PURCHASE_TYPE_OVERRIDES[recordId]){return purchaseTypeByKey(PURCHASE_TYPE_OVERRIDES[recordId]);}var text=name||'';var normalized=normalizeSearchText(text);var coursewareGeneric=false;var concreteMatch=false;for(var i=0;i<PURCHASE_TYPES.length;i++){var type=PURCHASE_TYPES[i];if(type.key==='unknownProduct'||type.key==='uncategorized'){continue;}for(var j=0;j<type.keywords.length;j++){var keyword=type.keywords[j];if(normalized.indexOf(normalizeSearchText(keyword))!==-1){if(type.key==='courseware'&&(keyword==='코스웨어'||keyword==='AI 코스웨어')){coursewareGeneric=true;}else{concreteMatch=true;}if(type.key!=='courseware'||keyword!=='코스웨어'){return type;}}}}if(coursewareGeneric){return purchaseTypeByKey('courseware');}var unknown=purchaseTypeByKey('unknownProduct');for(var u=0;u<unknown.keywords.length;u++){if(normalized.indexOf(normalizeSearchText(unknown.keywords[u]))!==-1&&!concreteMatch){return unknown;}}return purchaseTypeByKey('uncategorized');}
