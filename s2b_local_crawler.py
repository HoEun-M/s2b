...
def normalize_record_region_support(region, support_office):
    ...
    return normalized_region, normalize_support_office_name(normalized_region, text)
...
function rowSupportValue(row){var id=row.getAttribute('data-record-id');var regions=getRegionOverrides();var hasOverride=id&&Object.prototype.hasOwnProperty.call(regions,id);var region=rowRegionValue(row);var record=dashboardRecordForRow(row);if(record){var mapped=(region&&record.supportByRegion&&record.supportByRegion[region])?record.supportByRegion[region]:'';if(hasOverride&&mapped){return mapped;}if(record.supportOffice&&record.supportOffice!=='\uBBF8\uC9C0\uC815'){return record.supportOffice;}if(mapped){return mapped;}}var cell=row.querySelector('.support-cell');var value=cell?(cell.textContent||'').trim():'';return value||'\uBBF8\uC9C0\uC815';}
