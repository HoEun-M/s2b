...
function rowRegionValue(row){var fixed=row.querySelector('.fixed-region');if(fixed&&fixed.textContent.trim()){return fixed.textContent.trim();}var id=row.getAttribute('data-record-id');var regions=getRegionOverrides();if(id&&regions[id]){return regions[id];}var select=row.querySelector('.region-select');return select&&select.value?select.value:'\uBBF8\uC9C0\uC815';}
function rowSupportValue(row){var cell=row.querySelector('.support-cell');var value=cell?(cell.textContent||'').trim():'';return value||'\uBBF8\uC9C0\uC815';}
