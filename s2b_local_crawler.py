...
function rowSupportValue(row){var region=rowRegionValue(row);var record=dashboardRecordForRow(row);if(record){if(region&&record.supportByRegion&&record.supportByRegion[region]){return record.supportByRegion[region];}if(record.supportOffice){return record.supportOffice;}}var cell=row.querySelector('.support-cell');var value=cell?(cell.textContent||'').trim():'';return value||'\uBBF8\uC9C0\uC815';}
