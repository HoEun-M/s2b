...
var activeSource=(function(){try{return localStorage.getItem('s2b-source-v1')||'all';}catch(e){return 'all';}})();
var activeG2bKind='contract';
...
function updateSourceUi(){document.querySelectorAll('[data-source-btn]').forEach(function(btn){btn.classList.toggle('active',(btn.getAttribute('data-source-btn')||'all')===activeSource);});document.querySelectorAll('[data-g2b-kind-btn]').forEach(function(btn){btn.classList.toggle('active',(btn.getAttribute('data-g2b-kind-btn')||'')===activeG2bKind);});var chips=document.getElementById('kind-chips');if(chips){chips.style.display=activeSource==='s2b'?'none':'inline-flex';}var note=document.getElementById('source-note');if(note){note.textContent=activeSource==='s2b'?'':'대시보드 금액은 계약 건만 집계합니다 (입찰공고·낙찰 제외).';}}
function setSource(value){activeSource=value||'all';try{localStorage.setItem('s2b-source-v1',activeSource);}catch(e){}updateSourceUi();updateKeywordCounts();filterCurrent();renderDashboard();}
function setG2bKind(value){activeG2bKind=value||'contract';updateSourceUi();updateKeywordCounts();filterCurrent();}
...
""".strip()
    ...
    return (
        ...
        "<div class='source-bar'><span class='source-label'>출처</span>"
        "<button type='button' class='src-btn' data-source-btn='all' onclick=\"setSource('all')\">통합 <span class='cnt'>" + str(source_counts['s2b'] + source_counts['g2b']) + "</span></button>"
        "<button type='button' class='src-btn' data-source-btn='s2b' onclick=\"setSource('s2b')\">학교장터 <span class='cnt'>" + str(source_counts['s2b']) + "</span></button>"
        "<button type='button' class='src-btn' data-source-btn='g2b' onclick=\"setSource('g2b')\">나라장터 <span class='cnt'>" + str(source_counts['g2b']) + "</span></button>"
        "<span class='kind-chips' id='kind-chips'><span class='source-label'>나라장터 구분</span>"
        "<button type='button' class='src-btn small' data-g2b-kind-btn='contract' onclick=\"setG2bKind('contract')\">계약</button>"
        "<button type='button' class='src-btn small' data-g2b-kind-btn='bid' onclick=\"setG2bKind('bid')\">입찰공고</button>"
        "<button type='button' class='src-btn small' data-g2b-kind-btn='award' onclick=\"setG2bKind('award')\">낙찰</button></span>"
        "<span class='source-note' id='source-note'></span><a class='source-link' href='g2b.html'>나라장터 상세 보기 &rarr;</a></div>"
