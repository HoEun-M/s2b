...
var NAV_KEYS=['dashboard:all','dashboard:s2b','dashboard:g2b','details:all','details:s2b','details:g2b:contract','details:g2b:bid','details:g2b:award'];
...
function renderDashboard(updateRegionSelect){if(updateRegionSelect!==false){updateRegionSelect=true;}var rows=dashboardRows();var total=rows.reduce(function(sum,row){return sum+(Number(row.amount)||0);},0);var vendorMap={};var monthMap={};var regionMap={};var supportMap={};var schoolMap={};var schoolTypeMap={};var schoolOrder=['유치원','초등학교','중학교','고등학교','기타학교','기타기관'];var schoolTypeOrder=['선도','연구','중점','일반학교'];rows.forEach(function(row){addAmount(vendorMap,row.counterpart,row.amount);addAmount(monthMap,(row.contractDate||'').slice(0,7)||'\uBBF8\uC9C0\uC815',row.amount);addAmount(regionMap,row.region,row.amount);addAmount(schoolMap,row.schoolCategory||'기타기관',row.amount);addAmount(schoolTypeMap,schoolTypeLabel(row.schoolType),row.amount);});var vendorItems=sortedEntries(vendorMap);var monthItems=sortedEntries(monthMap).sort(function(a,b){return a.name.localeCompare(b.name);});var regionItems=sortedEntries(regionMap);var schoolItems=schoolOrder.map(function(name){return {name:name,value:schoolMap[name]||0};}).filter(function(item){return item.value>0;});var schoolTypeItems=schoolTypeOrder.map(function(name){return {name:name,value:schoolTypeMap[name]||0};}).filter(function(item){return item.value>0;});if(updateRegionSelect){renderRegionOptions(regionItems);}rows.forEach(function(row){if(activeDashboardRegion&&row.region===activeDashboardRegion){addAmount(supportMap,row.subregion||'미지정',row.amount);}});var supportItems=sortedEntries(supportMap).map(function(item){item.region=activeDashboardRegion;return item;});var totalEl=document.getElementById('dash-total');if(totalEl){totalEl.textContent=formatWon(total);}var countEl=document.getElementById('dash-count');if(countEl){countEl.textContent=rows.length.toLocaleString('ko-KR')+'\uAC74';}var topEl=document.getElementById('dash-top-vendor');if(topEl){topEl.textContent=vendorItems.length?vendorItems[0].name:'-';}var supportTitle=document.getElementById('support-title');if(supportTitle){supportTitle.textContent=(activeDashboardRegion||'\uC2DC\uB3C4')+' 세부지역별 매출액';}renderBars('vendor-chart',vendorItems,12,'vendor');renderBars('month-chart',monthItems,18,'month');renderBars('region-chart',regionItems,17,'region');renderBars('support-chart',supportItems,14,'subregion');renderBars('school-chart',schoolItems,6,'school');renderBars('school-type-chart',schoolTypeItems,4,'schoolType');}
...
function applyNavState(){var parts=activeNav.split(':');var view=parts[0];activeSource=parts[1]||'all';activeG2bKind=parts[2]||'contract';document.querySelectorAll('.view').forEach(function(el){el.classList.toggle('active',el.getAttribute('data-view')===view);});document.querySelectorAll('[data-nav]').forEach(function(btn){var key=btn.getAttribute('data-nav')||'';var active=key===activeNav||(key==='details:g2b'&&activeNav.indexOf('details:g2b:')===0);btn.classList.toggle('active',active);});var note=document.getElementById('source-note');if(note){note.textContent=activeSource==='s2b'?'':'대시보드 금액은 계약 건만 집계합니다 (나라장터 입찰공고·낙찰 제외).';note.style.display=activeSource==='s2b'?'none':'block';}}
function navigateTo(key){if(NAV_KEYS.indexOf(key)===-1){key='dashboard:all';}activeNav=key;try{localStorage.setItem('s2b-nav-v1',key);}catch(e){}applyNavState();updateKeywordCounts();if(key.indexOf('dashboard')===0){renderDashboard();}else{filterCurrent();}window.scrollTo({top:0});}
