...
SCHOOL_LOOKUP_ALIASES = {
    ...
    "강원대학교사범대부설고등학교": "강원대학교사범대학부설고등학교",
    "강원특별자치도교육청횡성여자고등학교": "횡성여자고등학교",
    "광주광일중학교": "광일중학교",
    "대구공업고등학교테크노폴리스캠퍼스": "대구공업고등학교테크노폴리스캠퍼스",
    "서울특별시교육청금호고등학교": "금호고등학교",
    "전북대사대부설고등학교": "전북대학교사범대학부설고등학교",
    "전주교육대전주부설초등학교": "전주교육대학교전주부설초등학교",
    "충북비지니스고등학교": "충북비즈니스고등학교",
}
SCHOOL_DISTRICT_OVERRIDES = {
    ...
    "경북기계공업고등학교부설대구산업학교": ("대구", "달서구"),
    "남양동화중학교": ("경기", "남양주시"),
    "대구공업고등학교테크노폴리스캠퍼스": ("대구", "달성군"),
    "밀양세종고등학교": ("경남", "밀양시"),
    "여주세종초등학교": ("경기", "여주시"),
    "인천경연초중학교": ("인천", "서구"),
    "인천청호초중학교": ("인천", "서구"),
...
GITHUB_UPLOAD_FILES = ("s2b_cumulative.json", "s2b_cumulative.html", "index.html", "school_type_mapping.json", "supabase_setup.sql", "crawl_ledger.json", "g2b_cumulative.json", "g2b.html")
...
_school_type_cache = None


def load_school_type_mapping():
    global _school_type_cache
    if _school_type_cache is not None:
        return _school_type_cache
    if not os.path.exists(SCHOOL_TYPE_MAPPING_FILE):
        _school_type_cache = {}
        return _school_type_cache
    try:
        with open(SCHOOL_TYPE_MAPPING_FILE, "r", encoding="utf-8") as file:
            data = json.load(file)
        _school_type_cache = data.get("labels", data if isinstance(data, dict) else {}) or {}
    except Exception as exc:
        print("[school-type] mapping load failed: " + str(exc))
        _school_type_cache = {}
    return _school_type_cache


def school_type(institution):
    normalized = normalize_school_name(institution)
    if not normalized:
        return ""
    mapping = load_school_type_mapping()
...
def support_office_unit_label(region, support_office):
    ...
    if region == "세종" and "세종" in text and "교육청" in text:
        return "세종"
...
def update_cumulative_json(results, date_from, date_to):
    ...
    for old in data.get("records", []):
        ...
        old["school_type"] = school_type(old.get("institution", ""))
...
G2B_CUMULATIVE_FILE = os.path.join(APP_DIR, "g2b_cumulative.json")
G2B_KIND_LABELS = {"contract": "계약", "bid": "입찰공고", "award": "낙찰"}


def load_g2b_rows():
    # 나라장터 누적(g2b_cumulative.json)을 학교장터 행과 같은 모양으로 바꿔 통합 페이지에 싣는다.
    if not os.path.exists(G2B_CUMULATIVE_FILE):
        return []
    try:
        with open(G2B_CUMULATIVE_FILE, "r", encoding="utf-8") as file:
            data = json.load(file)
    except Exception as exc:
        print("[html] 나라장터 누적 파일을 읽지 못해 통합에서 제외합니다: " + str(exc))
        return []
    rows = []
    for record in data.get("records", []):
        institution = record.get("institution") or record.get("contract_institution") or ""
        school_hint = record.get("school_name") or institution
        kind = record.get("source", "contract")
        rows.append({
            "id": record.get("id", ""),
            "tender_no": record.get("id", ""),
            "source": "g2b",
            "g2b_kind": kind,
            "g2b_label": G2B_KIND_LABELS.get(kind, kind),
            "contract_name": record.get("name", ""),
            "institution": institution,
            "school_name_hint": school_hint,
            "counterpart": record.get("counterpart", ""),
            "amount": record.get("amount", ""),
            "contract_date": record.get("date", ""),
            "keywords": record.get("keywords", []),
            "link": record.get("link", ""),
            "school_level": record.get("school_level", ""),
            "school_type": school_type(school_hint),
            "region": record.get("region", ""),
            "region_status": "direct" if record.get("region") else "",
            "region_source": "institution_name",
            "region_candidates": [],
            "support_office": "",
            "subregion": "",
...
def build_cumulative_html(data):
    ...
    backfill_record_subregions(records)
    for row in records:
        row.setdefault("source", "s2b")
    g2b_rows = load_g2b_rows()
    source_counts = {"s2b": len(records), "g2b": len(g2b_rows)}
    g2b_kind_counts = {"contract": 0, "bid": 0, "award": 0}
    for row in g2b_rows:
        g2b_kind_counts[row.get("g2b_kind", "contract")] = g2b_kind_counts.get(row.get("g2b_kind", "contract"), 0) + 1
    records = records + g2b_rows
    ...
    for index, row in enumerate(records, 1):
        ...
        if row.get("source") == "g2b":
            name_html = '<span class="tag src-badge">나라장터 · ' + esc(row.get("g2b_label", "")) + (' · ' + esc(row.get("method")) if row.get("method") else '') + '</span> ' + name_html
        search_text = " ".join(
            [
        ...
        school_type_value = row.get("school_type", "") or school_type(row.get("institution", ""))
        rows_html += (
            '<tr data-record-id="' + record_id + '" data-source="' + esc(row.get("source", "s2b")) + '" data-g2b-kind="' + esc(row.get("g2b_kind", "")) + '" data-keywords="' + esc(keywords_joined) + '" data-contract-name="' + esc(contract_name_raw) + '" data-search-text="' + esc(search_text) + '" data-level="' + esc(row.get("school_level", "")) + '" data-contract-date="' + esc(contract_date) + '" data-counterpart="' + esc(row.get("counterpart", "") or "미지정") + '" data-school-category="' + esc(school_category(row.get("school_name_hint") or row.get("institution", ""))) + '" data-school-type="' + esc(school_type_value) + '" data-subregion="' + esc(subregion or "미지정") + '"' + row_display_attr + '>'
            ...
            '<td class="tc school-type-cell">' + esc(school_type_value or "일반") + '</td>'
    ...
    for index, keyword in enumerate(all_keywords):
        ...
        if index == KEYWORD_VISIBLE:
            keyword_buttons += '<span id="kw-more" style="display:none">'
        keyword_buttons += (
    ...
    if len(all_keywords) > KEYWORD_VISIBLE:
        hidden = len(all_keywords) - KEYWORD_VISIBLE
        keyword_buttons += '</span><button type="button" class="btn kw-toggle" data-label-closed="더 보기 (' + str(hidden) + ')" onclick="toggleKeywordMore(this)">더 보기 (' + str(hidden) + ')</button>\n'
...
*{box-sizing:border-box}body{margin:0;font-family:'Nanum Gothic','Malgun Gothic',Arial,sans-serif;font-size:13px;color:#2f343b;background:#f4f6f8}.wrap{max-width:1280px;margin:0 auto;padding:24px 16px}.header{background:#245a92;color:#fff;padding:20px 24px;border-radius:8px;margin-bottom:16px}.header h1{font-size:19px;margin:0 0 7px}.meta{font-size:12px;opacity:.88}.panel,.toolbar{background:#fff;border:1px solid #dce4ec;border-radius:8px;margin-bottom:14px}.panel{padding:14px 16px}.panel h2{font-size:12px;color:#69727d;margin:0 0 10px}.filters,.toolbar{display:flex;flex-wrap:wrap;gap:8px;align-items:center}.btn{border:1px solid #2f6fa8;color:#245a92;background:#fff;border-radius:18px;padding:6px 12px;font-size:12px;cursor:pointer;font-family:inherit}.btn:hover{background:#eef5fb}.btn.active{background:#245a92;color:#fff}.cnt{background:rgba(36,90,146,.1);border-radius:10px;padding:1px 6px;margin-left:3px}.btn.active .cnt{background:rgba(255,255,255,.25)}.summary{display:flex;justify-content:space-between;gap:12px;align-items:center;background:#fff;border:1px solid #e2e6ea;border-radius:8px;padding:12px 16px;margin-bottom:14px}.summary strong{font-size:17px;color:#c0392b}.s2b-link{color:#245a92;text-decoration:none}.toolbar{padding:10px 12px}.toolbar-spacer{flex:1}.action-btn{height:30px;border:1px solid #245a92;border-radius:6px;background:#245a92;color:#fff;font-family:inherit;font-size:12px;padding:0 10px;cursor:pointer}.action-btn.danger{border-color:#b33a3a;background:#b33a3a}.action-btn.secondary{background:#fff;color:#245a92}.action-btn.filter-toggle{position:relative;border-color:#245a92;background:#fff;color:#245a92}.action-btn.filter-toggle.active{background:#245a92;color:#fff;box-shadow:0 0 0 3px rgba(36,90,146,.14)}.action-btn.filter-toggle.active::before{content:'ON';font-weight:700;margin-right:5px}.filter-state{display:inline-flex;align-items:center;height:30px;border:1px solid #d4e2ef;border-radius:15px;background:#f7fbff;color:#5c6670;padding:0 10px;font-size:12px}.filter-state.active{border-color:#245a92;background:#e8f1fa;color:#245a92;font-weight:600}.action-btn:hover{filter:brightness(.95)}.date-tools{gap:10px}.date-toggle{display:inline-flex;align-items:center;gap:5px;height:30px;border:1px solid #bfd0df;border-radius:16px;padding:0 10px;background:#fff;color:#263442;font-size:12px}.date-toggle input{margin:0}.date-input{height:30px;border:1px solid #b9c7d6;border-radius:6px;padding:0 8px;font-family:inherit;font-size:12px}.search-input{height:30px;min-width:220px;border:1px solid #b9c7d6;border-radius:6px;padding:0 9px;font-family:inherit;font-size:12px}.range-note{font-size:12px;color:#5c6670}.sync-status{font-size:12px;color:#5c6670}.sync-status.ok{color:#1d7a38}.sync-status.error{color:#b33a3a}.table-wrap{background:#fff;border:1px solid #e2e6ea;border-radius:8px;overflow:auto;max-height:calc(100vh - 260px);min-height:260px}table{width:100%;border-collapse:collapse;min-width:1180px}thead tr{background:#245a92;color:#fff}th{position:sticky;top:0;z-index:2;background:#245a92;padding:11px 9px;font-size:12px;font-weight:600;white-space:nowrap}td{padding:10px 9px;border-bottom:1px solid #edf0f2;vertical-align:middle}tbody tr{content-visibility:auto;contain-intrinsic-size:52px}tbody tr:hover td{background:#f8fbff}.tc{text-align:center}.tr{text-align:right}.select-cell{width:36px}.row-no{color:#89939e;width:42px}.contract-link{color:#1769aa;text-decoration:none}.contract-link:hover{text-decoration:underline}.tags{margin-top:5px}.tag{display:inline-block;background:#e8f1fa;color:#245a92;border-radius:10px;padding:1px 7px;font-size:11px;margin:2px 3px 0 0}.region-cell{min-width:210px}.school-type-cell{min-width:62px;white-space:nowrap;color:#245a92;font-weight:600}.region-editor{display:flex;gap:6px;justify-content:center;align-items:center}.region-select{height:28px;max-width:160px;border:1px solid #b9c7d6;border-radius:6px;background:#fff;color:#263442;font-family:inherit;font-size:12px;padding:0 6px}.region-save{height:28px;border:1px solid #245a92;border-radius:6px;background:#245a92;color:#fff;font-family:inherit;font-size:12px;padding:0 8px;cursor:pointer}.region-save:disabled,.action-btn:disabled{opacity:.65;cursor:wait}.region-text{font-weight:600;color:#263442}td:nth-child(9){font-weight:600;white-space:nowrap}td:nth-child(10){font-size:12px;color:#5c6670;white-space:nowrap}.no-result{text-align:center;padding:54px 20px;color:#8a94a0;display:none}.pagination{display:none;flex-wrap:wrap;gap:6px;align-items:center;justify-content:center;background:#fff;border:1px solid #e2e6ea;border-radius:8px;margin-top:10px;padding:10px}.pagination-top{position:sticky;top:0;z-index:5;margin-top:0;margin-bottom:10px;box-shadow:0 2px 8px rgba(36,90,146,.12)}.pagination-bottom{margin-top:10px}.page-btn{min-width:30px;height:30px;border:1px solid #bfd0df;border-radius:6px;background:#fff;color:#263442;font-family:inherit;font-size:12px;cursor:pointer}.page-btn:hover{background:#eef5fb}.page-btn.active{border-color:#245a92;background:#245a92;color:#fff}.page-btn:disabled{opacity:.45;cursor:not-allowed}.page-info{font-size:12px;color:#5c6670;margin-left:4px}.footer{text-align:center;color:#9aa3ad;font-size:11px;margin-top:18px}@media(max-width:720px){.wrap{padding:12px 8px}.header{padding:16px}.summary,.toolbar{align-items:flex-start;flex-direction:column}.panel{padding:12px}.table-wrap{max-height:calc(100vh - 220px)}.btn{padding:6px 10px}.region-editor{flex-direction:column}.region-select,.region-save{width:100%;max-width:none}}
""".strip()
    css += """
.admin-only{display:none!important}body.admin .admin-only{display:inline-flex!important}body:not(.admin) .select-cell{display:none}th.sortable{cursor:pointer;user-select:none}th.sortable:hover{background:#1f4f82}th.sorted{background:#1a4573}.sort-mark{font-size:10px;opacity:.9}.amount-strong{color:#245a92}.page-size{font-size:12px;color:#5c6670;display:inline-flex;align-items:center;gap:6px}.page-size .dash-select{height:28px}.date-presets{display:inline-flex;gap:6px;margin-left:6px;padding-left:10px;border-left:1px solid #e2e6ea}.kw-toggle{border-style:dashed}table{min-width:980px}td{padding:8px 6px}th{padding:10px 6px}.region-cell{min-width:150px}.region-select{max-width:120px}.subregion-cell,.school-type-cell{font-size:12px}td:nth-child(10){font-size:12px}.layout{display:flex;gap:16px;align-items:flex-start}.lnb{width:208px;flex:0 0 208px;position:sticky;top:12px;background:#fff;border:1px solid #dce4ec;border-radius:8px;padding:8px}.lnb-title{font-size:11px;color:#8a94a0;margin:8px 10px 4px;letter-spacing:.02em}.lnb-item{display:flex;justify-content:space-between;align-items:center;gap:8px;width:100%;border:0;background:transparent;text-align:left;padding:9px 10px;border-radius:6px;font-family:inherit;font-size:13px;color:#263442;cursor:pointer}.lnb-item:hover{background:#eef5fb}.lnb-item.active{background:#245a92;color:#fff}.lnb-item.active .cnt{background:rgba(255,255,255,.25)}.lnb-item.sub{padding-left:26px;font-size:12px}.lnb-item .cnt{margin-left:0}.lnb-link{display:block;margin:10px 10px 4px;font-size:12px;color:#245a92;text-decoration:none}.content{flex:1;min-width:0}.g2bcols-view iframe{width:100%;height:calc(100vh - 120px);min-height:520px;border:1px solid #dce4ec;border-radius:8px;background:#fff}.source-note{display:none;font-size:12px;color:#69727d;background:#fff;border:1px solid #dce4ec;border-radius:8px;padding:8px 12px;margin-bottom:12px}@media(max-width:900px){.layout{flex-direction:column}.lnb{position:static;width:100%;flex:none;display:flex;flex-wrap:wrap;gap:4px}.lnb-title{width:100%;margin:4px 6px}.lnb-item{width:auto}.lnb-item.sub{padding-left:10px}.lnb-link{width:100%}}.source-bar{display:flex;flex-wrap:wrap;gap:8px;align-items:center;background:#fff;border:1px solid #dce4ec;border-radius:8px;padding:10px 12px;margin:0 0 14px}.source-label{font-size:12px;color:#69727d;margin-right:2px}.src-btn{height:30px;border:1px solid #bfd0df;border-radius:15px;background:#fff;color:#263442;font-family:inherit;font-size:12px;padding:0 12px;cursor:pointer}.src-btn.active{border-color:#245a92;background:#245a92;color:#fff}.src-btn.small{height:26px;padding:0 10px;font-size:11px}.kind-chips{display:inline-flex;gap:6px;align-items:center;margin-left:6px;padding-left:10px;border-left:1px solid #e2e6ea}.source-note{font-size:12px;color:#69727d}.source-link{margin-left:auto;font-size:12px;color:#245a92;text-decoration:none}.src-badge{background:#fdecec;color:#b33a3a}.nav-tabs{display:flex;gap:8px;margin:0 0 14px}.tab-btn{height:34px;border:1px solid #bfd0df;border-radius:6px;background:#fff;color:#263442;font-family:inherit;font-size:13px;padding:0 14px;cursor:pointer}.tab-btn.active{border-color:#245a92;background:#245a92;color:#fff}.view{display:none}.view.active{display:block}.dashboard-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:14px}.dash-panel.span-2{grid-column:span 2}.dash-panel.span-3{grid-column:span 3}.dash-panel.span-4{grid-column:span 4}.month-chart{min-height:250px}.month-svg{display:block}.mc-bar rect{fill:#2f6fa8;cursor:pointer}.mc-bar:hover rect{fill:#245a92}.mc-val{font-size:10px;fill:#263442}.mc-x{font-size:11px;fill:#69727d}.mc-line{fill:none;stroke:#e67e22;stroke-width:2}.mc-dot{fill:#e67e22;stroke:#fff;stroke-width:1.5}.mc-legend{font-size:11px;color:#69727d;margin-top:6px;display:flex;gap:6px;align-items:center}.mc-leg-bar{display:inline-block;width:12px;height:10px;background:#2f6fa8;border-radius:2px}.mc-leg-line{display:inline-block;width:14px;height:2px;background:#e67e22;margin-left:8px}.donut-wrap{display:flex;flex-direction:column;gap:10px;align-items:center}.donut-wrap svg{flex:0 0 auto;width:168px;height:168px}.donut-legend{width:100%}.donut-seg{cursor:pointer;opacity:.92}.donut-seg:hover,.donut-seg.active{opacity:1;stroke:#263442;stroke-width:1.5}.donut-center{font-size:15px;font-weight:700;fill:#263442}.donut-center-sub{font-size:11px;fill:#69727d}.donut-legend{list-style:none;margin:0;padding:0;flex:1;min-width:0;display:flex;flex-direction:column;gap:4px}.donut-item{display:grid;grid-template-columns:10px 1fr auto auto;gap:8px;align-items:center;font-size:12px;color:#263442;cursor:pointer;padding:2px 4px;border-radius:4px}.donut-item:hover,.donut-item.active{background:#eef5fb}.donut-swatch{width:10px;height:10px;border-radius:2px;display:inline-block}.donut-label{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.donut-pct{color:#69727d;font-size:11px}.donut-amt{font-weight:600;white-space:nowrap}.purchase-details{margin-top:14px}.purchase-details summary{cursor:pointer;font-size:13px;color:#245a92;padding:10px 14px;background:#fff;border:1px solid #dce4ec;border-radius:8px;list-style:none}.purchase-details summary::-webkit-details-marker{display:none}.purchase-details summary::before{content:'▸ ';color:#69727d}.purchase-details[open] summary::before{content:'▾ '}.purchase-details .purchase-panel{margin-top:10px}.metric-row{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px;margin-bottom:14px}.metric-sub{font-size:11px;color:#8a94a0;margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.metric-sub.up{color:#1d7a38}.metric-sub.down{color:#b33a3a}.dash-filters{display:flex;flex-wrap:wrap;gap:8px;align-items:center;background:#fff;border:1px solid #dce4ec;border-radius:8px;padding:8px 12px;margin-bottom:12px}.dash-custom{display:inline-flex;gap:6px;align-items:center;margin-left:6px;padding-left:10px;border-left:1px solid #e2e6ea}.dash-custom .date-input{height:26px;font-size:11px}.dash-period-label{font-size:12px;color:#69727d;margin-left:4px}.chip{display:inline-flex;align-items:center;gap:6px;height:26px;border:1px solid #245a92;border-radius:13px;background:#e8f1fa;color:#245a92;padding:0 6px 0 10px;font-size:12px}.chip-x{border:0;background:transparent;color:#245a92;font-size:15px;line-height:1;cursor:pointer;padding:0 2px}.bar-fill.mine{background:#e67e22}.bar-label.mine{color:#c0392b;font-weight:700}.bar-rank{font-size:11px;color:#8a94a0;font-weight:400;margin-left:4px}.bar-share{font-size:11px;color:#8a94a0;font-weight:400;margin-left:5px}@media(max-width:1200px){.metric-row{grid-template-columns:repeat(3,minmax(0,1fr))}}.metric{background:#fff;border:1px solid #e2e6ea;border-radius:8px;padding:14px 16px}.metric-label{font-size:12px;color:#69727d;margin-bottom:6px}.metric-value{font-size:22px;font-weight:700;color:#263442}.dash-panel{background:#fff;border:1px solid #e2e6ea;border-radius:8px;padding:14px 16px;min-width:0}.dash-panel h2{font-size:13px;color:#263442;margin:0 0 12px}.dash-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px}.dash-head h2{margin:0}.dash-select{height:30px;border:1px solid #b9c7d6;border-radius:6px;background:#fff;color:#263442;font-family:inherit;font-size:12px;padding:0 8px}.bar-list{display:flex;flex-direction:column;gap:8px}.bar-row{display:grid;grid-template-columns:minmax(88px,160px) 1fr minmax(90px,auto);gap:10px;align-items:center;width:100%;border:0;background:transparent;padding:0;font-family:inherit;text-align:left}.bar-row-button{cursor:pointer}.bar-row-button:hover .bar-label{color:#1769aa;text-decoration:underline}.bar-row-button:focus-visible{outline:2px solid #245a92;outline-offset:2px;border-radius:4px}.bar-label{font-size:12px;color:#263442;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.bar-track{height:10px;border-radius:999px;background:#e8eef4;overflow:hidden}.bar-fill{height:100%;border-radius:999px;background:#2f6fa8}.bar-value{font-size:12px;color:#263442;text-align:right;font-weight:600;white-space:nowrap}.dashboard-filter-note{display:none;align-items:center;justify-content:space-between;gap:10px;background:#fff;border:1px solid #dce4ec;border-radius:8px;margin-bottom:14px;padding:10px 12px;color:#263442}.dashboard-filter-note strong{color:#245a92}.empty-chart{padding:28px 8px;color:#8a94a0;text-align:center}.detail-view .panel{margin-top:0}@media(max-width:900px){.dashboard-grid,.metric-row{grid-template-columns:1fr}.dash-panel.span-2,.dash-panel.span-3,.dash-panel.span-4{grid-column:span 1}.donut-wrap{flex-direction:column}.bar-row{grid-template-columns:1fr}.bar-value{text-align:left}.dashboard-filter-note{align-items:flex-start;flex-direction:column}}
""".strip()
    css += """
...
""".strip()
    css += """
.subregion-cell{min-width:90px;white-space:nowrap;color:#263442}tbody td:nth-child(9){font-weight:600;white-space:nowrap}tbody td:nth-child(10){font-size:12px;color:#5c6670;white-space:nowrap}.purchase-clear{height:28px;border:1px solid #245a92;border-radius:14px;background:#fff;color:#245a92;font-family:inherit;font-size:12px;padding:0 10px;cursor:pointer}.purchase-clear.active{background:#245a92;color:#fff}
""".strip()

    js = """
var activeKeyword='all';
var NAV_KEYS=['dashboard:all','dashboard:s2b','dashboard:g2b','details:all','details:s2b','details:g2b:contract','details:g2b:bid','details:g2b:award','g2bcols'];
var activeNav=(function(){try{var v=localStorage.getItem('s2b-nav-v1');return NAV_KEYS.indexOf(v)!==-1?v:'dashboard:all';}catch(e){return 'dashboard:all';}})();
var activeSource=activeNav.split(':')[1]||'all';
var activeG2bKind=activeNav.split(':')[2]||'contract';
...
var schoolTypeStorageKey='s2b-school-type-overrides-v1';
...
var schoolTypeCache=null;
var currentPage=1;
var pageSize=(function(){try{var v=parseInt(localStorage.getItem('s2b-page-size-v1'),10);return [20,50,100].indexOf(v)!==-1?v:20;}catch(e){return 20;}})();
var sortKey='date';var sortDir='desc';
var adminMode=(function(){try{return localStorage.getItem('s2b-admin-mode-v1')==='1';}catch(e){return false;}})();
function setPageSize(value){pageSize=parseInt(value,10)||20;try{localStorage.setItem('s2b-page-size-v1',String(pageSize));}catch(e){}currentPage=1;filterCurrent(true);}
function setAdminMode(value){adminMode=!!value;try{localStorage.setItem('s2b-admin-mode-v1',adminMode?'1':'0');}catch(e){}document.body.classList.toggle('admin',adminMode);var btn=document.getElementById('admin-toggle');if(btn){btn.classList.toggle('active',adminMode);btn.textContent=adminMode?'관리 모드 켜짐':'관리 모드';}}
function rowSortValue(row,key){if(key==='amount'){return amountFromRow(row);}if(key==='date'){return row.getAttribute('data-contract-date')||'';}if(key==='name'){return row.getAttribute('data-contract-name')||'';}if(key==='counterpart'){return row.getAttribute('data-counterpart')||'';}if(key==='institution'){var cell=row.querySelector('td:nth-child(7)');return cell?cell.textContent.trim():'';}if(key==='region'){return rowRegionValue(row)+' '+rowSubregionValue(row);}return '';}
function setSort(key){if(sortKey===key){sortDir=sortDir==='asc'?'desc':'asc';}else{sortKey=key;sortDir=(key==='amount'||key==='date')?'desc':'asc';}updateSortUi();filterCurrent();}
function updateSortUi(){document.querySelectorAll('th[data-sort]').forEach(function(th){var key=th.getAttribute('data-sort');var active=key===sortKey;th.classList.toggle('sorted',active);var mark=th.querySelector('.sort-mark');if(mark){mark.textContent=active?(sortDir==='asc'?'▲':'▼'):'';}});}
function applyDatePreset(kind){var today=new Date();var from;var to=today;if(kind==='yesterday'){from=new Date(today.getFullYear(),today.getMonth(),today.getDate()-1);to=from;}else if(kind==='week'){var dow=(today.getDay()+6)%7;from=new Date(today.getFullYear(),today.getMonth(),today.getDate()-dow);}else if(kind==='month'){from=new Date(today.getFullYear(),today.getMonth(),1);}else{return;}var fromEl=document.getElementById('date-from-filter');var toEl=document.getElementById('date-to-filter');if(fromEl){fromEl.value=isoDate(from);}if(toEl){toEl.value=isoDate(to);}applyDateSearch();}
function toggleKeywordMore(btn){var more=document.getElementById('kw-more');if(!more){return;}var open=more.style.display!=='none';more.style.display=open?'none':'contents';btn.textContent=open?btn.getAttribute('data-label-closed'):'접기';}
...
{key:'assessment',label:'평가도구',keywords:['지니아튜터','매쓰홀릭','매쓰홀릭T','스쿨플랫','매쓰플랫','수학대왕','기출탭탭','일프로연산','문제은행','진단평가','학력진단','단원평가','리드 인공지능 문해력 진단','수학 학습 프로그램','평가지원','평가']},
{key:'courseware',label:'코스웨어',keywords:['코들','Codle','초코팝','초코클래스','달달','옥수수','홈런','아이스크림홈런','밀크티','스마트올','엘리하이','자작자작','클래스팅','클래스팅 AI','토도수학','알공 수학','똑똑수학탐험대','토도한글','러니','레서','매일국어T','리드','토도영어','알공 영어','쿠키영어','리딩앤','리딩게이트','리딩오션','원아워','영어독후활동프로그램','코드모스','엘리스스쿨','엘리스 LXP','플랭','플랭스쿨','스쿨런','두드림','AI 코스웨어','코스웨어']},
{key:'teacherAi',label:'교사업무지원 AI',keywords:['마이클','MyClass','My Class','이음AI','이음 AI','AI 구독','AI구독','인라인AI','인라인 AI','InlineAI','Inline AI','유튜브','YouTube','GPT','지피티','챗지피티','클로드','Claude','제미나이','Gemini','캡컷','CapCut','진로진학상담','수파베이스','Supabase','supabase','커서','Cursor','Google AI Pro','구글 AI','Grok']},
{key:'tool',label:'수업도구',keywords:['젠스파크','Genspark','학습지원','Zoom','줌','Vrew','vrew','브루','캔바','Canva','미리캔버스','젭','ZEP','카훗','Kahoot','북크리에이터','패들렛','Padlet','감마','Gamma','수노','Suno','뤼튼','노션','Notion','띵커벨','퀴즈앤','클래스카드','퀴즈렛','다했니','슬라이도','망고보드','투닝','스픽','Speak','일레븐랩스','ElevenLabs','런웨이','Runway','미드저니','Midjourney','교우관계','화상영어','메타버스','클링AI','클링','공학도구','심스페이스','마음일기','AI대화','감정사전','마음관리']},
{key:'hardware',label:'하드웨어·디바이스',keywords:['태블릿','태블릿PC','태블릿 PC','태블릿컴퓨터','태블릿 컴퓨터','노트북','전자칠판','충전함','실물화상기','실물 화상기','크롬북','아이패드','거치대','갤럭시탭','갤럭시 탭','S10FE','S11','키보드','마우스','카메라','웹캠','모니터','스마트기기','디바이스','비스포크','무선청소기','청소기','AI제트','청정스테이션','텔레비전','냉장고']},
...
const PURCHASE_TYPE_OVERRIDES={
'202608031409153':'tool',
'202607161381940':'tool',
'202607161380648':'tool',
'202606301340573':'tool',
...
function getSchoolTypeOverrides(){if(schoolTypeCache===null){schoolTypeCache=readJsonStorage(schoolTypeStorageKey,{});}return schoolTypeCache;}
function setSchoolTypeOverrides(value){schoolTypeCache=value||{};writeJsonStorage(schoolTypeStorageKey,schoolTypeCache);}
...
async function loadSupabaseSchoolTypes(){var response=await supabaseFetch('/rest/v1/school_type_overrides?select=record_id,school_type&limit=20000',{method:'GET'});var rows=await response.json();var map={};rows.forEach(function(row){if(row.record_id&&row.school_type){map[row.record_id]=row.school_type;}});return map;}
...
function updateRegionalFields(row,region){updateSubregion(row,region);}
...
function applySchoolTypes(types){types=types||{};dashboardRecords.forEach(function(record){if(record.id&&types[record.id]){record.schoolType=types[record.id];}});document.querySelectorAll('#tbody tr').forEach(function(row){var id=row.getAttribute('data-record-id');var value=(id&&types[id])?types[id]:(row.getAttribute('data-school-type')||'');row.setAttribute('data-school-type',value);var cell=row.querySelector('.school-type-cell');if(cell){cell.textContent=value||'일반';}});}
function updateKeywordCounts(){var rows=Array.from(document.querySelectorAll('#tbody tr')).filter(function(row){return row.getAttribute('data-deleted')!=='1'&&rowMatchesSource(row);});document.querySelectorAll('[data-kind="keyword"]').forEach(function(btn){var value=btn.getAttribute('data-filter-value')||'all';var count=value==='all'?rows.length:rows.filter(function(row){var keywords=row.getAttribute('data-keywords')||'';return (','+keywords+',').indexOf(','+value+',')!==-1;}).length;var cnt=btn.querySelector('.cnt');if(cnt){cnt.textContent=count.toLocaleString('ko-KR');}});}
...
async function loadRemoteState(){try{var localRegions=getRegionOverrides();var localDeleted=getDeletedRecords();var localSchoolTypes=getSchoolTypeOverrides();var regionRemote=await loadSupabaseRegions();var deletedRemote=await loadSupabaseDeleted();var schoolTypeRemote={};try{schoolTypeRemote=await loadSupabaseSchoolTypes();}catch(typeError){schoolTypeRemote={};}var mergedRegions=Object.assign({},regionRemote,localRegions);var mergedDeleted=Array.from(new Set((deletedRemote||[]).concat(localDeleted||[]))).sort();var mergedSchoolTypes=Object.assign({},schoolTypeRemote,localSchoolTypes);setRegionOverrides(mergedRegions);setDeletedRecords(mergedDeleted);setSchoolTypeOverrides(mergedSchoolTypes);applyRegions(mergedRegions);applySchoolTypes(mergedSchoolTypes);applyDeleted(mergedDeleted);setStatus('Supabase data loaded.','ok');}catch(error){applyRegions(getRegionOverrides());applySchoolTypes(getSchoolTypeOverrides());applyDeleted(getDeletedRecords());setStatus('Supabase 연결 실패. 로컬 저장 모드로 동작합니다.','error');}}
...
function showView(name){if(name==='dashboard'){navigateTo('dashboard:'+activeSource);return;}navigateTo(activeSource==='g2b'?'details:g2b:'+activeG2bKind:'details:'+activeSource);}
function formatWon(value){return (Number(value)||0).toLocaleString('ko-KR')+'\uC6D0';}
function formatWonShort(value){var n=Number(value)||0;var sign=n<0?'-':'';n=Math.abs(n);if(n>=100000000){var eok=n/100000000;return sign+(eok>=100?Math.round(eok).toLocaleString('ko-KR'):eok.toFixed(1))+'억';}if(n>=10000){return sign+Math.round(n/10000).toLocaleString('ko-KR')+'만';}return sign+n.toLocaleString('ko-KR')+'원';}
var DASH_PERIODS={month:'이번 달',m3:'최근 3개월',year:'올해',all:'전체',custom:'직접 입력'};
var dashPeriod=(function(){try{var v=localStorage.getItem('s2b-dash-period-v1');return DASH_PERIODS[v]?v:'all';}catch(e){return 'all';}})();
var dashFrom='';var dashTo='';
function pad2(n){return (n<10?'0':'')+n;}
function isoDate(d){return d.getFullYear()+'-'+pad2(d.getMonth()+1)+'-'+pad2(d.getDate());}
function computeDashRange(){var today=new Date();var y=today.getFullYear();var m=today.getMonth();if(dashPeriod==='month'){return [isoDate(new Date(y,m,1)),isoDate(today)];}if(dashPeriod==='m3'){return [isoDate(new Date(y,m-2,1)),isoDate(today)];}if(dashPeriod==='year'){return [y+'-01-01',isoDate(today)];}if(dashPeriod==='custom'){return [dashFrom,dashTo];}return ['',''];}
var dashExcludeLarge=(function(){try{return localStorage.getItem('s2b-dash-large-v1')==='1';}catch(e){return false;}})();
var DASH_LARGE_THRESHOLD=1000000000;
function setDashExcludeLarge(value){dashExcludeLarge=!!value;try{localStorage.setItem('s2b-dash-large-v1',dashExcludeLarge?'1':'0');}catch(e){}renderDashboard();}
function recordInDashPeriod(record){if(dashExcludeLarge&&(Number(record.amount)||0)>=DASH_LARGE_THRESHOLD){return false;}var range=computeDashRange();var date=record.contractDate||'';if(range[0]&&(!date||date<range[0])){return false;}if(range[1]&&(!date||date>range[1])){return false;}return true;}
function dashPeriodLabel(){var range=computeDashRange();if(!range[0]&&!range[1]){return '전체 기간';}return (range[0]||'')+' ~ '+(range[1]||'');}
function setDashPeriod(value){if(!DASH_PERIODS[value]){value='all';}if(value==='custom'){var from=document.getElementById('dash-from');var to=document.getElementById('dash-to');dashFrom=from?from.value:'';dashTo=to?to.value:'';if(!dashFrom&&!dashTo){setStatus('기간을 입력하세요.','error');return;}if(dashFrom&&dashTo&&dashFrom>dashTo){var t=dashFrom;dashFrom=dashTo;dashTo=t;}}dashPeriod=value;try{localStorage.setItem('s2b-dash-period-v1',value);}catch(e){}renderDashboard();}
function updateDashFilterUi(){document.querySelectorAll('[data-period]').forEach(function(btn){btn.classList.toggle('active',(btn.getAttribute('data-period')||'')===dashPeriod);});var large=document.getElementById('dash-large');if(large){large.checked=dashExcludeLarge;}var label=document.getElementById('dash-period-label');if(label){label.textContent=dashPeriodLabel();}var chip=document.getElementById('dash-detail-chip');if(chip){if(dashboardDetailFilter){chip.style.display='inline-flex';chip.innerHTML='<span>세부내역 필터: '+escapeHtml(dashboardFilterLabel(dashboardDetailFilter))+'</span><button type="button" class="chip-x" onclick="clearDashboardFilter();updateDashFilterUi();" aria-label="필터 해제">&times;</button>';}else{chip.style.display='none';chip.innerHTML='';}}}
...
var countMaps={};
function addAmount(map,key,amount){key=key||'미지정';map[key]=(map[key]||0)+(Number(amount)||0);if(!countMaps.__current){return;}var cm=countMaps.__current;cm[key]=(cm[key]||0)+1;}
function sortedEntries(map,counts){return Object.keys(map).map(function(key){return {name:key,value:map[key],count:counts?(counts[key]||0):0};}).sort(function(a,b){return b.value-a.value||a.name.localeCompare(b.name,'ko-KR');});}
...
function purchaseTypeTooltip(item){var desc={aidt:'AI 디지털 교육자료, AI 디지털 교과서, 발행사 AIDT 계약입니다. 많이 보이는 품목: AIDT, 디지털교육자료, AI디지털교과서.',assessment:'진단, 평가, 문제은행, 연산 드릴형 도구입니다. 많이 보이는 품목: 지니아튜터, 매쓰홀릭, 스쿨플랫, 매쓰플랫, 수학대왕, 진단평가.',courseware:'교과 학습형 코스웨어입니다. 많이 보이는 품목: 초코, 홈런, 밀크티, 스마트올, 토도수학, 리딩앤, 플랭, 코스웨어.',teacherAi:'교사의 업무, 콘텐츠 제작, 검색, 상담, AI 활용을 돕는 도구입니다. 많이 보이는 품목: GPT, Claude, Gemini, 유튜브, 캡컷, 커서, Supabase, 진로진학상담.',tool:'수업 진행, 활동, 발표, 협업, 생성형 AI 활용을 돕는 수업도구입니다. 많이 보이는 품목: Canva, Padlet, Kahoot, ZEP, Zoom, Vrew, 메타버스, 클링, 심스페이스.',hardware:'수업이나 학교 운영에 쓰이는 하드웨어와 디바이스입니다. 많이 보이는 품목: 태블릿, 노트북, 전자칠판, 크롬북, 청소기, TV, 냉장고.',coding:'코딩, SW, 개발 학습 관련 도구입니다. 많이 보이는 품목: 코딩, Coding, 퓨너스.',material:'실물 교구와 체험형 자료입니다. 많이 보이는 품목: 레고, 스파이크, 4D프레임, 보드게임, 키트, 드론, 로봇, VR/AR.',book:'교재, 도서, 워크북, 문제집류입니다. 많이 보이는 품목: EBS, 만점왕, 워크북, 문제집, 학습지, 교재 세트.',service:'연수, 컨설팅, 강사, 운영 용역입니다. 많이 보이는 품목: 교원연수, 직무연수, 강사, 컨설팅, 운영용역.',uncategorized:'아직 사전 규칙에 걸리지 않은 항목입니다. 목록을 확인해 반복 키워드를 새 유형 규칙에 추가하는 후보입니다.'};var key=item&&item.key?item.key:'uncategorized';return (item&&item.label?item.label:'미분류')+'\\n'+(desc[key]||desc.uncategorized);}
...
function renderBars(id,items,limit,filterType,pinMine){var el=document.getElementById(id);if(!el){return;}var all=items||[];var shown=all.slice(0,limit||12);if(pinMine){var mine=all.filter(function(item){return isMineName(item.name);});mine.forEach(function(item){if(shown.indexOf(item)===-1){item.rank=all.indexOf(item)+1;shown.push(item);}});}if(!shown.length){el.innerHTML='<div class="empty-chart">표시할 매출액이 없습니다.</div>';return;}var total=all.reduce(function(sum,item){return sum+(Number(item.value)||0);},0)||1;var max=Math.max.apply(null,shown.map(function(item){return item.value;}))||1;el.innerHTML=shown.map(function(item){var pct=Math.max(2,Math.round(item.value/max*100));var share=(item.value/total*100);var mineClass=isMineName(item.name)?' mine':'';var label=escapeHtml(item.name)+(item.rank?' <span class="bar-rank">'+item.rank+'위</span>':'');var tip=escapeHtml(item.name+' · '+formatWon(item.value)+' · '+share.toFixed(1)+'%'+(item.count?' · '+item.count.toLocaleString('ko-KR')+'건':''));var click=filterType?' onclick="applyDashboardFilter('+escapeHtml(JSON.stringify(filterType))+','+escapeHtml(JSON.stringify(item.name))+','+escapeHtml(JSON.stringify(item.region||''))+')"':'';return '<button type="button" class="bar-row '+(filterType?'bar-row-button':'')+'" title="'+tip+'"'+click+'><div class="bar-label'+mineClass+'">'+label+'</div><div class="bar-track"><div class="bar-fill'+mineClass+'" style="width:'+pct+'%"></div></div><div class="bar-value">'+formatWonShort(item.value)+'<span class="bar-share">'+share.toFixed(0)+'%</span></div></button>';}).join('');}
function dashboardRows(){var regions=getRegionOverrides();var deleted=new Set(getDeletedRecords()||[]);var schoolTypes=getSchoolTypeOverrides();return dashboardRecords.filter(function(row){return !deleted.has(row.id)&&recordMatchesSource(row)&&recordInDashPeriod(row);}).map(function(row){var hasOverride=Object.prototype.hasOwnProperty.call(regions,row.id);var region=hasOverride?regions[row.id]:(row.region||'\uBBF8\uC9C0\uC815');var original=row.supportOffice||'';var mapped=(region&&row.supportByRegion&&row.supportByRegion[region])?row.supportByRegion[region]:'';var support=(hasOverride&&mapped)?mapped:((original&&original!=='\uBBF8\uC9C0\uC815')?original:(mapped||'\uBBF8\uC9C0\uC815'));if(!supportMatchesRegion(region,support)){support=mapped||'\uBBF8\uC9C0\uC815';}var subregionMapped=(region&&row.subregionByRegion&&row.subregionByRegion[region])?row.subregionByRegion[region]:'';var subregion=(hasOverride&&subregionMapped)?subregionMapped:(row.subregion||subregionMapped||'\uBBF8\uC9C0\uC815');var schoolType=(row.id&&schoolTypes[row.id])?schoolTypes[row.id]:(row.schoolType||'');return Object.assign({},row,{region:region,supportOffice:support,subregion:subregion,schoolType:schoolType});});}
...
var CHART_COLORS=['#245a92','#2f6fa8','#5b8fc7','#e67e22','#f0a35c','#1d7a38','#5cb85c','#8e44ad','#b085d6','#b33a3a','#d9534f','#7f8c8d','#95a5a6'];
function monthLabel(name){return name&&name.length>=7?name.slice(2,4)+'.'+name.slice(5,7):name;}
function svgText(tag,attrs,content){return '<'+tag+' '+attrs+'>'+content+'</'+tag+'>';}
function renderMonthChart(items){var el=document.getElementById('month-chart');if(!el){return;}var present=(items||[]).filter(function(item){return item.name&&item.name!=='미지정'&&/^[0-9]{4}-[0-9]{2}$/.test(item.name);});if(!present.length){el.innerHTML='<div class="empty-chart">표시할 매출액이 없습니다.</div>';return;}var byName={};present.forEach(function(item){byName[item.name]=item;});var first=present[0].name;var last=present[present.length-1].name;var data=[];var y=parseInt(first.slice(0,4),10);var m=parseInt(first.slice(5,7),10);while(true){var key=y+'-'+(m<10?'0':'')+m;data.push(byName[key]||{name:key,value:0,count:0});if(key>=last){break;}m+=1;if(m>12){m=1;y+=1;}if(data.length>120){break;}}data=data.slice(-12);var W=Math.max(320,el.clientWidth||600);var H=250;var padL=8;var padR=8;var padT=30;var padB=30;var n=data.length;var maxV=Math.max.apply(null,data.map(function(d){return d.value;}))||1;var maxC=Math.max.apply(null,data.map(function(d){return d.count||0;}))||1;var slot=(W-padL-padR)/n;var bw=Math.max(10,Math.min(46,slot*0.55));var plotH=H-padT-padB;var bars='';var points=[];data.forEach(function(d,i){var h=Math.max(2,d.value/maxV*plotH);var x=padL+i*slot+(slot-bw)/2;var y=H-padB-h;var cx=padL+i*slot+slot/2;var cy=H-padB-((d.count||0)/maxC*plotH*0.85);points.push([cx,cy,d.count||0]);var tip=escapeHtml(d.name+' · '+formatWon(d.value)+' · '+(d.count||0).toLocaleString('ko-KR')+'건');bars+='<g class="mc-bar" data-month="'+escapeHtml(d.name)+'"><title>'+tip+'</title>'+'<rect x="'+x.toFixed(1)+'" y="'+y.toFixed(1)+'" width="'+bw.toFixed(1)+'" height="'+h.toFixed(1)+'" rx="3"></rect>'+'<text class="mc-val" x="'+cx.toFixed(1)+'" y="'+(y-6).toFixed(1)+'" text-anchor="middle">'+escapeHtml(formatWonShort(d.value))+'</text>'+'<text class="mc-x" x="'+cx.toFixed(1)+'" y="'+(H-padB+16)+'" text-anchor="middle">'+escapeHtml(monthLabel(d.name))+'</text></g>';});var line='<polyline class="mc-line" points="'+points.map(function(p){return p[0].toFixed(1)+','+p[1].toFixed(1);}).join(' ')+'"></polyline>'+points.map(function(p){return '<circle class="mc-dot" cx="'+p[0].toFixed(1)+'" cy="'+p[1].toFixed(1)+'" r="3.5"><title>'+p[2].toLocaleString('ko-KR')+'건</title></circle>';}).join('');el.innerHTML='<svg class="month-svg" viewBox="0 0 '+W+' '+H+'" width="100%" height="'+H+'" preserveAspectRatio="none">'+bars+line+'</svg>'+'<div class="mc-legend"><span class="mc-leg-bar"></span>매출액 <span class="mc-leg-line"></span>건수 · 최근 '+n+'개월 · 막대 클릭 시 세부내역</div>';el.querySelectorAll('.mc-bar').forEach(function(g){g.addEventListener('click',function(){applyDashboardFilter('month',g.getAttribute('data-month')||'','');});});}
function donutArc(cx,cy,r,inner,a0,a1){var large=(a1-a0)>Math.PI?1:0;var p=function(rad,a){return (cx+rad*Math.cos(a)).toFixed(2)+' '+(cy+rad*Math.sin(a)).toFixed(2);};return 'M'+p(r,a0)+' A'+r+' '+r+' 0 '+large+' 1 '+p(r,a1)+' L'+p(inner,a1)+' A'+inner+' '+inner+' 0 '+large+' 0 '+p(inner,a0)+' Z';}
function renderDonut(rows){var el=document.getElementById('purchase-type-chart');if(!el){return;}var data=(rows||[]).filter(function(item){return item.amount>0;});var total=data.reduce(function(sum,item){return sum+item.amount;},0);if(!data.length||!total){el.innerHTML='<div class="empty-chart">표시할 매출액이 없습니다.</div>';return;}var cx=92;var cy=92;var r=84;var inner=54;var angle=-Math.PI/2;var paths='';var legend='';data.forEach(function(item,i){var frac=item.amount/total;var a1=angle+frac*Math.PI*2;var color=CHART_COLORS[i%CHART_COLORS.length];var active=activePurchaseType===item.key;var tip=escapeHtml(item.label+' · '+formatWon(item.amount)+' · '+(frac*100).toFixed(1)+'% · '+item.count.toLocaleString('ko-KR')+'건');if(frac>=0.999){paths+='<circle cx="'+cx+'" cy="'+cy+'" r="'+((r+inner)/2)+'" fill="none" stroke="'+color+'" stroke-width="'+(r-inner)+'"><title>'+tip+'</title></circle>';}else{paths+='<path class="donut-seg'+(active?' active':'')+'" d="'+donutArc(cx,cy,r,inner,angle,a1-0.004)+'" fill="'+color+'" data-key="'+escapeHtml(item.key)+'"><title>'+tip+'</title></path>';}angle=a1;legend+='<li class="donut-item'+(active?' active':'')+'" data-key="'+escapeHtml(item.key)+'" title="'+tip+'"><span class="donut-swatch" style="background:'+color+'"></span><span class="donut-label">'+escapeHtml(item.label)+'</span><span class="donut-pct">'+(frac*100).toFixed(0)+'%</span><span class="donut-amt">'+escapeHtml(formatWonShort(item.amount))+'</span></li>';});var center='<text x="'+cx+'" y="'+(cy-4)+'" text-anchor="middle" class="donut-center">'+escapeHtml(formatWonShort(total))+'</text><text x="'+cx+'" y="'+(cy+14)+'" text-anchor="middle" class="donut-center-sub">'+data.reduce(function(s,i){return s+i.count;},0).toLocaleString('ko-KR')+'건</text>';el.innerHTML='<div class="donut-wrap"><svg viewBox="0 0 184 184" width="184" height="184">'+paths+center+'</svg><ul class="donut-legend">'+legend+'</ul></div><div class="purchase-note">세부내역에 걸린 필터 기준 · 조각 클릭 시 해당 유형만 보기</div>';el.querySelectorAll('[data-key]').forEach(function(node){node.addEventListener('click',function(){setPurchaseTypeFilter(node.getAttribute('data-key')||'');});});}
...
function schoolTypeLabel(value){return value||'일반학교';}
function renderDashboard(updateRegionSelect){if(updateRegionSelect!==false){updateRegionSelect=true;}updateDashFilterUi();var rows=dashboardRows();var total=rows.reduce(function(sum,row){return sum+(Number(row.amount)||0);},0);var vendorMap={};var monthMap={};var regionMap={};var supportMap={};var schoolMap={};var schoolTypeMap={};var vendorCounts={};var monthCounts={};var regionCounts={};var schoolOrder=['유치원','초등학교','중학교','고등학교','기타학교','기타기관'];var schoolTypeOrder=['선도','연구','중점','일반학교'];rows.forEach(function(row){countMaps.__current=vendorCounts;addAmount(vendorMap,row.counterpart,row.amount);countMaps.__current=monthCounts;addAmount(monthMap,(row.contractDate||'').slice(0,7)||'미지정',row.amount);countMaps.__current=regionCounts;addAmount(regionMap,row.region,row.amount);countMaps.__current=null;addAmount(schoolMap,row.schoolCategory||'기타기관',row.amount);addAmount(schoolTypeMap,schoolTypeLabel(row.schoolType),row.amount);});countMaps.__current=null;var vendorItems=sortedEntries(vendorMap,vendorCounts);var monthItems=sortedEntries(monthMap,monthCounts).sort(function(a,b){return a.name.localeCompare(b.name);});var regionItems=sortedEntries(regionMap,regionCounts);var schoolItems=schoolOrder.map(function(name){return {name:name,value:schoolMap[name]||0};}).filter(function(item){return item.value>0;});var schoolTypeItems=schoolTypeOrder.map(function(name){return {name:name,value:schoolTypeMap[name]||0};}).filter(function(item){return item.value>0;});if(updateRegionSelect){renderRegionOptions(regionItems);}rows.forEach(function(row){if(activeDashboardRegion&&row.region===activeDashboardRegion){addAmount(supportMap,row.subregion||'미지정',row.amount);}});var supportItems=sortedEntries(supportMap).map(function(item){item.region=activeDashboardRegion;return item;});var totalEl=document.getElementById('dash-total');if(totalEl){totalEl.textContent=formatWonShort(total);totalEl.title=formatWon(total);}var countEl=document.getElementById('dash-count');if(countEl){countEl.textContent=rows.length.toLocaleString('ko-KR')+'건';}var avgEl=document.getElementById('dash-avg');if(avgEl){var avg=rows.length?Math.round(total/rows.length):0;avgEl.textContent=formatWonShort(avg);avgEl.title=formatWon(avg);}var allRows=dashboardRecords.filter(function(row){return recordMatchesSourceOnly(row)&&!(new Set(getDeletedRecords()||[])).has(row.id);});var now=new Date();var thisKey=now.getFullYear()+'-'+pad2(now.getMonth()+1);var prevDate=new Date(now.getFullYear(),now.getMonth()-1,1);var prevKey=prevDate.getFullYear()+'-'+pad2(prevDate.getMonth()+1);var thisSum=0;var prevSum=0;allRows.forEach(function(row){var key=(row.contractDate||'').slice(0,7);if(key===thisKey){thisSum+=Number(row.amount)||0;}else if(key===prevKey){prevSum+=Number(row.amount)||0;}});var momEl=document.getElementById('dash-mom');var momSub=document.getElementById('dash-mom-sub');if(momEl){momEl.textContent=formatWonShort(thisSum);momEl.title=formatWon(thisSum);}if(momSub){var diff=prevSum?((thisSum-prevSum)/prevSum*100):null;momSub.textContent=(now.getMonth()+1)+'월 · 전월 '+formatWonShort(prevSum)+(diff===null?'':' · '+(diff>=0?'+':'')+diff.toFixed(0)+'%');momSub.className='metric-sub '+(diff===null?'':(diff>=0?'up':'down'));}var mineSum=0;rows.forEach(function(row){if(isMineName(row.counterpart)){mineSum+=Number(row.amount)||0;}});var mineEl=document.getElementById('dash-mine');var mineSub=document.getElementById('dash-mine-sub');if(mineEl){mineEl.textContent=(total?(mineSum/total*100).toFixed(1):'0.0')+'%';mineEl.title='미래엔 '+formatWon(mineSum)+' / 전체 '+formatWon(total);}if(mineSub){var mineRank=0;vendorItems.forEach(function(item,index){if(!mineRank&&isMineName(item.name)){mineRank=index+1;}});mineSub.textContent='미래엔 '+formatWonShort(mineSum)+(mineRank?' · '+mineRank+'위':'')+(vendorItems.length?' · 1위 '+vendorItems[0].name:'');}var periodEl=document.getElementById('dash-total-sub');if(periodEl){periodEl.textContent=dashPeriodLabel()+(activeSource==='s2b'?'':' · 계약 건만 집계');}var supportTitle=document.getElementById('support-title');if(supportTitle){supportTitle.textContent=(activeDashboardRegion||'\uC2DC\uB3C4')+' 세부지역별 매출액';}renderBars('vendor-chart',vendorItems,12,'vendor',true);renderMonthChart(monthItems);renderBars('region-chart',regionItems,17,'region');renderBars('support-chart',supportItems,14,'subregion');renderBars('school-chart',schoolItems,6,'school');renderBars('school-type-chart',schoolTypeItems,4,'schoolType');}
...
function rowSubregionValue(row){var region=rowRegionValue(row);var record=dashboardRecordForRow(row);if(record){var mapped=(region&&record.subregionByRegion&&record.subregionByRegion[region])?record.subregionByRegion[region]:'';if(mapped){return mapped;}if(record.subregion){return record.subregion;}}var cell=row.querySelector('.subregion-cell');var value=cell?(cell.textContent||'').trim():(row.getAttribute('data-subregion')||'');return value||'\uBBF8\uC9C0\uC815';}
function rowMatchesDashboardFilter(row){if(!dashboardDetailFilter){return true;}var value=dashboardDetailFilter.value||'';if(dashboardDetailFilter.type==='vendor'){return (row.getAttribute('data-counterpart')||'\uBBF8\uC9C0\uC815')===value;}if(dashboardDetailFilter.type==='month'){return ((row.getAttribute('data-contract-date')||'').slice(0,7)||'\uBBF8\uC9C0\uC815')===value;}if(dashboardDetailFilter.type==='school'){return (row.getAttribute('data-school-category')||'\uAE30\uD0C0\uAE30\uAD00')===value;}if(dashboardDetailFilter.type==='schoolType'){return schoolTypeLabel(row.getAttribute('data-school-type')||'')===value;}if(dashboardDetailFilter.type==='region'){return rowRegionValue(row)===value;}if(dashboardDetailFilter.type==='subregion'){return rowSubregionValue(row)===value&&(!dashboardDetailFilter.region||rowRegionValue(row)===dashboardDetailFilter.region);}return true;}
function dashboardFilterLabel(filter){if(!filter){return '';}var names={vendor:'업체',month:'월',school:'학교급',schoolType:'학교구분',region:'시도',subregion:'세부지역'};var prefix=names[filter.type]||'대시보드';return prefix+': '+filter.value+(filter.type==='subregion'&&filter.region?' ('+filter.region+')':'');}
...
function filterCurrent(keepPage){if(!keepPage){resetPage();}updateUnsavedUi();updateDashboardFilterNote();var rows=document.querySelectorAll('#tbody tr');var matches=[];var purchaseStats=createPurchaseStats();rows.forEach(function(row){var keywords=row.getAttribute('data-keywords')||'';var keywordMatch=activeKeyword==='all'||(','+keywords+',').indexOf(','+activeKeyword+',')!==-1;var contractText=normalizeSearchText(row.getAttribute('data-search-text')||row.getAttribute('data-contract-name')||'');var contractMatch=!contractSearchText||contractText.indexOf(contractSearchText)!==-1;var deleted=row.getAttribute('data-deleted')==='1';var unsavedMatch=!unsavedOnly||!rowHasRegion(row);var dashboardMatch=rowMatchesDashboardFilter(row);var type=classifyPurchaseType(row.getAttribute('data-contract-name')||'',row.getAttribute('data-record-id')||'');row.setAttribute('data-purchase-type',type.key);var purchaseMatch=!activePurchaseType||type.key===activePurchaseType;var vendorScopeMatch=vendorMatchesScope(row);var matched=keywordMatch&&contractMatch&&!deleted&&unsavedMatch&&rowMatchesDate(row)&&dashboardMatch&&purchaseMatch&&vendorScopeMatch&&rowMatchesSource(row);row.setAttribute('data-filter-match',matched?'1':'0');if(matched){matches.push(row);addPurchaseStats(purchaseStats,row,type,amountFromRow(row));}});renderPurchaseStats(purchaseStats);if(sortKey){var dir=sortDir==='asc'?1:-1;matches.sort(function(a,b){var va=rowSortValue(a,sortKey);var vb=rowSortValue(b,sortKey);if(typeof va==='number'&&typeof vb==='number'){return (va-vb)*dir;}return String(va).localeCompare(String(vb),'ko-KR')*dir;});}var sumAmount=matches.reduce(function(sum,row){return sum+amountFromRow(row);},0);var amountEl=document.getElementById('visible-amount');if(amountEl){amountEl.textContent=formatWonShort(sumAmount);amountEl.title=formatWon(sumAmount);}var total=matches.length;var pages=Math.max(1,Math.ceil(total/pageSize));if(currentPage>pages){currentPage=pages;}var start=(currentPage-1)*pageSize;var end=start+pageSize;rows.forEach(function(row){row.style.display='none';var cb=row.querySelector('.row-check');if(cb){cb.checked=false;}var no=row.querySelector('.row-no');if(no){no.textContent='';}});var tbodyEl=document.getElementById('tbody');matches.slice(start,end).forEach(function(row,index){if(tbodyEl){tbodyEl.appendChild(row);}row.style.display='';var no=row.querySelector('.row-no');if(no){no.textContent=start+index+1;}});document.getElementById('visible-count').textContent=total.toLocaleString('ko-KR');document.getElementById('no-result').style.display=total===0?'block':'none';renderPagination(total);var master=document.querySelector('thead input[type="checkbox"]');if(master){master.checked=false;}}
function rowMatchesSource(row){var src=row.getAttribute('data-source')||'s2b';if(activeSource!=='all'&&src!==activeSource){return false;}if(src==='g2b'&&(row.getAttribute('data-g2b-kind')||'contract')!==activeG2bKind){return false;}return true;}
function recordMatchesSource(record){var src=record.source||'s2b';if(activeSource!=='all'&&src!==activeSource){return false;}return src!=='g2b'||(record.kind||'contract')==='contract';}
function recordMatchesSourceOnly(record){return recordMatchesSource(record);}
function isMineName(name){return normalizeSearchText(name||'').indexOf(normalizeSearchText('미래엔'))!==-1;}
function applyNavState(){var parts=activeNav.split(':');var view=parts[0];if(view!=='g2bcols'){activeSource=parts[1]||'all';activeG2bKind=parts[2]||'contract';}else{var frame=document.getElementById('g2b-frame');var kind=(activeSource==='g2b'&&activeG2bKind)?activeG2bKind:'contract';var src='g2b.html?embed=1&source='+kind;if(frame&&frame.getAttribute('data-src')!==src){frame.setAttribute('data-src',src);frame.src=src;}}document.querySelectorAll('.view').forEach(function(el){el.classList.toggle('active',el.getAttribute('data-view')===view);});document.querySelectorAll('[data-nav]').forEach(function(btn){var key=btn.getAttribute('data-nav')||'';var active=key===activeNav||(key==='details:g2b'&&(activeNav.indexOf('details:g2b:')===0||activeNav==='g2bcols'));btn.classList.toggle('active',active);});var note=document.getElementById('source-note');if(note){note.textContent='';note.style.display='none';}}
function navigateTo(key){if(NAV_KEYS.indexOf(key)===-1){key='dashboard:all';}activeNav=key;try{localStorage.setItem('s2b-nav-v1',key);}catch(e){}applyNavState();if(key==='g2bcols'){window.scrollTo({top:0});return;}updateKeywordCounts();if(key.indexOf('dashboard')===0){renderDashboard();}else{filterCurrent();}window.scrollTo({top:0});}
function updateSourceUi(){applyNavState();}
...
document.addEventListener('DOMContentLoaded',function(){setAdminMode(adminMode);updateSortUi();var ps=document.getElementById('page-size');if(ps){ps.value=String(pageSize);}updateSourceUi();syncDateToggles();applyRegions(getRegionOverrides());applySchoolTypes(getSchoolTypeOverrides());applyDeleted(getDeletedRecords());loadRemoteState();});
""".strip()
    ...
    return (
        ...
        "<div class='wrap'>"
        "<div class='header'><h1>학교장터 · 나라장터 교육기관 계약 누적 내역</h1>"
        "<div class='meta'>&#45572;&#51201; &#49373;&#49457;: " + esc(exported_at) + " &middot; <a href='g2b.html' style='color:#fff'>나라장터 교육기관 입찰·계약 보기 &rarr;</a></div></div>"
        "<div class='layout'><nav class='lnb' aria-label='메뉴'>"
        "<div class='lnb-title'>대시보드</div>"
        "<button type='button' class='lnb-item' data-nav='dashboard:all' onclick=\"navigateTo('dashboard:all')\"><span>통합</span><span class='cnt'>" + "{:,}".format(source_counts['s2b'] + g2b_kind_counts['contract']) + "</span></button>"
        "<button type='button' class='lnb-item' data-nav='dashboard:s2b' onclick=\"navigateTo('dashboard:s2b')\"><span>S2B 학교장터</span><span class='cnt'>" + "{:,}".format(source_counts['s2b']) + "</span></button>"
        "<button type='button' class='lnb-item' data-nav='dashboard:g2b' onclick=\"navigateTo('dashboard:g2b')\"><span>G2B 나라장터</span><span class='cnt'>" + "{:,}".format(g2b_kind_counts['contract']) + "</span></button>"
        "<div class='lnb-title'>세부내역</div>"
        "<button type='button' class='lnb-item' data-nav='details:all' onclick=\"navigateTo('details:all')\"><span>통합 세부내역</span><span class='cnt'>" + "{:,}".format(source_counts['s2b'] + source_counts['g2b']) + "</span></button>"
        "<button type='button' class='lnb-item' data-nav='details:s2b' onclick=\"navigateTo('details:s2b')\"><span>S2B 세부내역</span><span class='cnt'>" + "{:,}".format(source_counts['s2b']) + "</span></button>"
        "<button type='button' class='lnb-item' data-nav='details:g2b' onclick=\"navigateTo('details:g2b:contract')\"><span>G2B 세부내역</span><span class='cnt'>" + "{:,}".format(source_counts['g2b']) + "</span></button>"
        "<button type='button' class='lnb-item sub' data-nav='details:g2b:bid' onclick=\"navigateTo('details:g2b:bid')\"><span>입찰공고</span><span class='cnt'>" + "{:,}".format(g2b_kind_counts['bid']) + "</span></button>"
        "<button type='button' class='lnb-item sub' data-nav='details:g2b:contract' onclick=\"navigateTo('details:g2b:contract')\"><span>계약</span><span class='cnt'>" + "{:,}".format(g2b_kind_counts['contract']) + "</span></button>"
        "<button type='button' class='lnb-item sub' data-nav='details:g2b:award' onclick=\"navigateTo('details:g2b:award')\"><span>낙찰</span><span class='cnt'>" + "{:,}".format(g2b_kind_counts['award']) + "</span></button>"
        "<button type='button' class='lnb-item sub' data-nav='g2bcols' onclick=\"navigateTo('g2bcols')\"><span>전용 컬럼 보기</span><span class='cnt'>&rarr;</span></button>"
        "</nav><div class='content'>"
        "<section class='view active' data-view='dashboard'>"
        "<div class='dash-filters'><span class='source-label'>기간</span>"
        "<button type='button' class='src-btn small' data-period='month' onclick=\"setDashPeriod('month')\">이번 달</button>"
        "<button type='button' class='src-btn small' data-period='m3' onclick=\"setDashPeriod('m3')\">최근 3개월</button>"
        "<button type='button' class='src-btn small' data-period='year' onclick=\"setDashPeriod('year')\">올해</button>"
        "<button type='button' class='src-btn small' data-period='all' onclick=\"setDashPeriod('all')\">전체</button>"
        "<span class='dash-custom'><input type='date' id='dash-from' class='date-input'> ~ <input type='date' id='dash-to' class='date-input'><button type='button' class='src-btn small' data-period='custom' onclick=\"setDashPeriod('custom')\">적용</button></span>"
        "<span class='dash-period-label' id='dash-period-label'></span><label class='date-toggle' title='나라장터의 대형 시스템 구축 계약 등 10억 원 이상 건을 집계에서 뺍니다'><input type='checkbox' id='dash-large' onchange='setDashExcludeLarge(this.checked)'> 10억 이상 제외</label><span class='chip' id='dash-detail-chip' style='display:none'></span></div>"
        "<div class='metric-row'>"
        "<div class='metric'><div class='metric-label'>총매출액</div><div class='metric-value' id='dash-total'>0원</div><div class='metric-sub' id='dash-total-sub'></div><div class='metric-sub' id='source-note' style='display:none'></div></div>"
        "<div class='metric'><div class='metric-label'>계약 건수</div><div class='metric-value' id='dash-count'>0건</div></div>"
        "<div class='metric'><div class='metric-label'>건당 평균</div><div class='metric-value' id='dash-avg'>0원</div></div>"
        "<div class='metric'><div class='metric-label'>이번 달 매출</div><div class='metric-value' id='dash-mom'>0원</div><div class='metric-sub' id='dash-mom-sub'></div></div>"
        "<div class='metric'><div class='metric-label'>미래엔 점유율</div><div class='metric-value' id='dash-mine'>0%</div><div class='metric-sub' id='dash-mine-sub'></div></div>"
        "</div>"
        "<div class='dashboard-grid'><div class='dash-panel span-4'><h2>월별 매출 추이</h2><div id='month-chart' class='month-chart'></div></div><div class='dash-panel span-2'><div class='dash-head'><h2>구매 유형별 계약금액</h2><button type='button' class='purchase-clear' data-purchase-clear='1' onclick='clearPurchaseTypeFilter()'>전체로 보기</button></div><div id='purchase-type-chart'></div></div><div class='dash-panel span-3'><h2>업체별 매출액</h2><div class='bar-list' id='vendor-chart'></div></div><div class='dash-panel span-3'><h2>시도별 매출액</h2><div class='bar-list' id='region-chart'></div></div><div class='dash-panel span-2'><div class='dash-head'><h2 id='support-title'>시도 세부지역별 매출액</h2><select id='dashboard-region' class='dash-select' onchange='setDashboardRegion(this.value)'></select></div><div class='bar-list' id='support-chart'></div></div><div class='dash-panel span-2'><h2>학교급별 매출액</h2><div class='bar-list' id='school-chart'></div></div><div class='dash-panel span-2'><h2>학교구분별 매출액</h2><div class='bar-list' id='school-type-chart'></div></div></div><details class='purchase-details'><summary>구매 유형 상세 표 · 유형 × 학교급 · 유형 × 월별 추이 · CSV</summary><div class='purchase-panel'><div class='purchase-head'><h2>구매 유형별 계약금액</h2><div class='purchase-tools'><button type='button' class='purchase-clear' data-purchase-clear='1' onclick='clearPurchaseTypeFilter()'>전체로 보기</button><button type='button' class='purchase-scope active' data-purchase-scope='all' onclick=\"setPurchaseVendorScope('all')\">전체</button><button type='button' class='purchase-scope' data-purchase-scope='mirae' onclick=\"setPurchaseVendorScope('mirae')\">미래엔</button><button type='button' class='purchase-scope' data-purchase-scope='other' onclick=\"setPurchaseVendorScope('other')\">그 외</button><button type='button' class='purchase-csv' onclick='downloadPurchaseCsv()'>CSV 다운로드</button></div></div><div class='purchase-table-wrap'><table class='purchase-table'><thead><tr><th>유형</th><th class='tr'>건수</th><th class='tr'>계약금액 합</th><th class='tr'>비중</th><th class='tr'>건당 평균</th></tr></thead><tbody id='purchase-type-body'></tbody><tfoot id='purchase-type-foot'></tfoot></table></div><div id='purchase-complex-note' class='purchase-note'>복합계약 0건 포함 (단일 제품 기준 아님)</div><div class='purchase-subgrid'><div class='purchase-subpanel'><h3>유형 × 학교급</h3><div class='purchase-table-wrap'><table class='purchase-table'><thead><tr><th>유형</th><th class='tr'>초</th><th class='tr'>중</th><th class='tr'>고</th><th class='tr'>기타</th></tr></thead><tbody id='purchase-level-body'></tbody></table></div></div><div class='purchase-subpanel'><h3>유형 × 월별 추이</h3><div class='purchase-table-wrap'><table class='purchase-table'><thead id='purchase-month-head'></thead><tbody id='purchase-month-body'></tbody></table></div></div></div></div></details></section>"
        ...
        "<div class='summary'><span>총 <strong id='visible-count'>" + str(default_visible_count) + "</strong>건 · 합계 <strong id='visible-amount' class='amount-strong'>0원</strong></span>"
        "<label class='page-size'>페이지당 <select id='page-size' class='dash-select' onchange='setPageSize(this.value)'><option value='20'>20</option><option value='50'>50</option><option value='100'>100</option></select>건</label>"
        "<a href='" + esc(ref_url) + "' target='_blank' rel='noopener' class='s2b-link'>S2B &#49688;&#51032;&#44228;&#50557; &#45236;&#50669; &#48148;&#47196;&#44032;&#44592; &#8599;</a></div>"
        "<div class='toolbar date-tools'><label class='date-toggle'><input type='checkbox' id='all-view' checked onchange='setAllView(this.checked)'>&#51204;&#52404; &#48372;&#44592;</label><input type='search' id='contract-search' class='search-input' placeholder='&#44228;&#50557;&#47749;&#183;&#44228;&#50557;&#44592;&#44288;&#183;&#44228;&#50557;&#45824;&#49345;&#51088; &#44160;&#49353;' aria-label='&#44228;&#50557;&#47749;&#183;&#44228;&#50557;&#44592;&#44288;&#183;&#44228;&#50557;&#45824;&#49345;&#51088; &#44160;&#49353;' oninput='setContractSearch(this.value)'><button type='button' class='action-btn secondary' onclick='clearContractSearch()'>&#44160;&#49353; &#52488;&#44592;&#54868;</button><input type='date' id='date-from-filter' class='date-input' aria-label='&#44228;&#50557;&#52404;&#44208;&#51068; &#49884;&#51089;&#51068;'><span class='range-note'>~</span><input type='date' id='date-to-filter' class='date-input' aria-label='&#44228;&#50557;&#52404;&#44208;&#51068; &#51333;&#47308;&#51068;'><button type='button' class='action-btn secondary' onclick='applyDateSearch()'>&#45216;&#51676; &#44160;&#49353;</button><button type='button' class='action-btn secondary' onclick='clearDateSearch()'>&#52488;&#44592;&#54868;</button><span class='date-presets'><button type='button' class='src-btn small' onclick=\"applyDatePreset('yesterday')\">어제</button><button type='button' class='src-btn small' onclick=\"applyDatePreset('week')\">이번 주</button><button type='button' class='src-btn small' onclick=\"applyDatePreset('month')\">이번 달</button></span></div>"
        "<div class='toolbar'><button type='button' id='admin-toggle' class='action-btn filter-toggle' onclick='setAdminMode(!adminMode)' title='삭제·지역 저장 동기화 같은 관리 기능을 켜고 끕니다'>관리 모드</button>"
        "<button type='button' id='delete-selected' class='action-btn danger admin-only' onclick='deleteSelected()'>&#49440;&#53469; &#49325;&#51228;</button>"
        "<button type='button' id='unsaved-only-btn' class='action-btn filter-toggle admin-only' aria-pressed='false' onclick='toggleUnsavedOnly()'>&#51648;&#50669; &#48120;&#51200;&#51109;&#47564; &#48372;&#44592;</button>"
        "<button type='button' id='sync-local' class='action-btn secondary admin-only' onclick='syncLocalToSupabase(this)'>&#47196;&#52972; &#51200;&#51109; &#46041;&#44592;&#54868;</button>"
        ...
        "<th class='select-cell'><input type='checkbox' aria-label='전체 선택' onclick='toggleAll(this)'></th><th>No</th>"
        "<th style='text-align:left' class='sortable' data-sort='name' onclick=\"setSort('name')\">계약명 <span class='sort-mark'></span></th>"
        "<th class='sortable' data-sort='region' onclick=\"setSort('region')\">지역 <span class='sort-mark'></span></th><th>세부지역</th><th>학교구분</th>"
        "<th class='sortable' data-sort='institution' onclick=\"setSort('institution')\">계약기관 <span class='sort-mark'></span></th>"
        "<th class='sortable' data-sort='counterpart' onclick=\"setSort('counterpart')\">계약대상자 <span class='sort-mark'></span></th>"
        "<th class='sortable tr' data-sort='amount' onclick=\"setSort('amount')\">금액 <span class='sort-mark'></span></th>"
        "<th class='sortable' data-sort='date' onclick=\"setSort('date')\">계약체결일 <span class='sort-mark'></span></th>"
        ...
        "<section class='view g2bcols-view' data-view='g2bcols'><iframe id='g2b-frame' title='나라장터 전용 컬럼' src='about:blank' loading='lazy'></iframe></section>"
        "</div></div>"
