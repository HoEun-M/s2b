...
EXCLUDE_WORDS = [
    ...
    "예술", "옥수수", "외국어", "외국어학교", "재료", "전자칠판", "화이트보드", "체육",
...
def build_cumulative_html(data):
    ...
    for index, row in enumerate(records, 1):
        ...
        rows_html += (
            '<tr data-record-id="' + record_id + '" data-keywords="' + esc(keywords_joined) + '" data-contract-name="' + esc(contract_name_raw) + '" data-level="' + esc(row.get("school_level", "")) + '" data-contract-date="' + esc(contract_date) + '"' + row_display_attr + '>'
    ...
    for row in records:
        ...
        dashboard_records.append({"id": row.get("id") or row.get("tender_no", ""), "counterpart": row.get("counterpart", "") or "미지정", "amount": amount_number(row.get("amount", "")), "contractDate": row.get("contract_date", ""), "region": row.get("region", "") or "미지정", "supportOffice": row.get("support_office", "") or "미지정", "supportByRegion": support_by_region})
...
function addAmount(map,key,amount){key=key||'???';map[key]=(map[key]||0)+(Number(amount)||0);}
...
function renderBars(id,items,limit){var el=document.getElementById(id);if(!el){return;}var shown=(items||[]).slice(0,limit||12);if(!shown.length){el.innerHTML='<div class="empty-chart">??? ???? ????.</div>';return;}var max=Math.max.apply(null,shown.map(function(item){return item.value;}))||1;el.innerHTML=shown.map(function(item){var pct=Math.max(2,Math.round(item.value/max*100));return '<div class="bar-row"><div class="bar-label" title="'+item.name+'">'+item.name+'</div><div class="bar-track"><div class="bar-fill" style="width:'+pct+'%"></div></div><div class="bar-value">'+formatWon(item.value)+'</div></div>';}).join('');}
function dashboardRows(){var regions=getRegionOverrides();var deleted=new Set(getDeletedRecords()||[]);return dashboardRecords.filter(function(row){return !deleted.has(row.id);}).map(function(row){var region=regions[row.id]||row.region||'???';var support=(region&&row.supportByRegion&&row.supportByRegion[region])?row.supportByRegion[region]:(row.supportOffice||'???');return Object.assign({},row,{region:region,supportOffice:support});});}
function renderRegionOptions(items){var select=document.getElementById('dashboard-region');if(!select){return;}var current=activeDashboardRegion;var regions=items.map(function(item){return item.name;}).filter(function(name){return name&&name!=='???';});select.innerHTML='<option value="">?? ??</option>'+regions.map(function(name){return '<option value="'+name+'">'+name+'</option>';}).join('');if(current&&regions.indexOf(current)!==-1){select.value=current;}else{activeDashboardRegion=regions[0]||'';select.value=activeDashboardRegion;}}
function setDashboardRegion(value){activeDashboardRegion=value;renderDashboard(false);}
function renderDashboard(updateRegionSelect){if(updateRegionSelect!==false){updateRegionSelect=true;}var rows=dashboardRows();var total=rows.reduce(function(sum,row){return sum+(Number(row.amount)||0);},0);var vendorMap={};var monthMap={};var regionMap={};var supportMap={};rows.forEach(function(row){addAmount(vendorMap,row.counterpart,row.amount);addAmount(monthMap,(row.contractDate||'').slice(0,7)||'???',row.amount);addAmount(regionMap,row.region,row.amount);});var vendorItems=sortedEntries(vendorMap);var monthItems=sortedEntries(monthMap).sort(function(a,b){return a.name.localeCompare(b.name);});var regionItems=sortedEntries(regionMap);if(updateRegionSelect){renderRegionOptions(regionItems);}rows.forEach(function(row){if(activeDashboardRegion&&row.region===activeDashboardRegion){addAmount(supportMap,row.supportOffice,row.amount);}});var supportItems=sortedEntries(supportMap);var totalEl=document.getElementById('dash-total');if(totalEl){totalEl.textContent=formatWon(total);}var countEl=document.getElementById('dash-count');if(countEl){countEl.textContent=rows.length.toLocaleString('ko-KR')+'?';}var topEl=document.getElementById('dash-top-vendor');if(topEl){topEl.textContent=vendorItems.length?vendorItems[0].name:'-';}var supportTitle=document.getElementById('support-title');if(supportTitle){supportTitle.textContent=(activeDashboardRegion||'??')+' ?????? ???';}renderBars('vendor-chart',vendorItems,12);renderBars('month-chart',monthItems,18);renderBars('region-chart',regionItems,17);renderBars('support-chart',supportItems,14);}
...
function filterCurrent(keepPage){if(!keepPage){resetPage();}updateUnsavedUi();var rows=document.querySelectorAll('#tbody tr');var matches=[];rows.forEach(function(row){var keywords=row.getAttribute('data-keywords')||'';var keywordMatch=activeKeyword==='all'||(','+keywords+',').indexOf(','+activeKeyword+',')!==-1;var contractText=normalizeSearchText(row.getAttribute('data-contract-name')||'');var contractMatch=!contractSearchText||contractText.indexOf(contractSearchText)!==-1;var deleted=row.getAttribute('data-deleted')==='1';var unsavedMatch=!unsavedOnly||!rowHasRegion(row);var matched=keywordMatch&&contractMatch&&!deleted&&unsavedMatch&&rowMatchesDate(row);row.setAttribute('data-filter-match',matched?'1':'0');if(matched){matches.push(row);}});var total=matches.length;var pages=Math.max(1,Math.ceil(total/pageSize));if(currentPage>pages){currentPage=pages;}var start=(currentPage-1)*pageSize;var end=start+pageSize;rows.forEach(function(row){row.style.display='none';var cb=row.querySelector('.row-check');if(cb){cb.checked=false;}var no=row.querySelector('.row-no');if(no){no.textContent='';}});matches.slice(start,end).forEach(function(row,index){row.style.display='';var no=row.querySelector('.row-no');if(no){no.textContent=start+index+1;}});document.getElementById('visible-count').textContent=total;document.getElementById('no-result').style.display=total===0?'block':'none';renderPagination(total);var master=document.querySelector('thead input[type="checkbox"]');if(master){master.checked=false;}}
