...
function filterCurrent(keepPage){if(!keepPage){resetPage();}updateUnsavedUi();updateDashboardFilterNote();var rows=document.querySelectorAll('#tbody tr');var matches=[];var purchaseStats=createPurchaseStats();rows.forEach(function(row){var keywords=row.getAttribute('data-keywords')||'';var keywordMatch=activeKeyword==='all'||(','+keywords+',').indexOf(','+activeKeyword+',')!==-1;var contractText=normalizeSearchText(row.getAttribute('data-search-text')||row.getAttribute('data-contract-name')||'');var contractMatch=!contractSearchText||contractText.indexOf(contractSearchText)!==-1;var deleted=row.getAttribute('data-deleted')==='1';var unsavedMatch=!unsavedOnly||!rowHasRegion(row);var dashboardMatch=rowMatchesDashboardFilter(row);var type=classifyPurchaseType(row.getAttribute('data-contract-name')||'',row.getAttribute('data-record-id')||'');row.setAttribute('data-purchase-type',type.key);var purchaseMatch=!activePurchaseType||type.key===activePurchaseType;var vendorScopeMatch=vendorMatchesScope(row);var matched=keywordMatch&&contractMatch&&!deleted&&unsavedMatch&&rowMatchesDate(row)&&dashboardMatch&&purchaseMatch&&vendorScopeMatch&&rowMatchesSource(row);row.setAttribute('data-filter-match',matched?'1':'0');if(matched){matches.push(row);addPurchaseStats(purchaseStats,row,type,amountFromRow(row));}});renderPurchaseStats(purchaseStats);var total=matches.length;var pages=Math.max(1,Math.ceil(total/pageSize));if(currentPage>pages){currentPage=pages;}var start=(currentPage-1)*pageSize;var end=start+pageSize;rows.forEach(function(row){row.style.display='none';var cb=row.querySelector('.row-check');if(cb){cb.checked=false;}var no=row.querySelector('.row-no');if(no){no.textContent='';}});matches.slice(start,end).forEach(function(row,index){row.style.display='';var no=row.querySelector('.row-no');if(no){no.textContent=start+index+1;}});document.getElementById('visible-count').textContent=total;document.getElementById('no-result').style.display=total===0?'block':'none';renderPagination(total);var master=document.querySelector('thead input[type="checkbox"]');if(master){master.checked=false;}}
...
document.addEventListener('DOMContentLoaded',function(){updateSourceUi();syncDateToggles();applyRegions(getRegionOverrides());applySchoolTypes(getSchoolTypeOverrides());applyDeleted(getDeletedRecords());loadRemoteState();});
""".strip()
    ...
    return (
        ...
        "<th><input type='checkbox' aria-label='&#51204;&#52404; &#49440;&#53469;' onclick='toggleAll(this)'></th><th>No</th><th style='text-align:left'>&#44228;&#50557;&#47749;</th><th>&#51648;&#50669;</th><th>&#49464;&#48512;&#51648;&#50669;</th><th>학교구분</th><th>&#44228;&#50557;&#44592;&#44288;</th><th>&#44228;&#50557;&#45824;&#49345;&#51088;</th>"
