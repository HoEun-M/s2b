...
def build_cumulative_html(data):
    ...
    for index, row in enumerate(records, 1):
        ...
        rows_html += (
            '<tr data-record-id="' + record_id + '" data-keywords="' + esc(keywords_joined) + '" data-contract-name="' + esc(contract_name_raw) + '" data-search-text="' + esc(search_text) + '" data-level="' + esc(row.get("school_level", "")) + '" data-contract-date="' + esc(contract_date) + '" data-counterpart="' + esc(row.get("counterpart", "") or "미지정") + '" data-school-category="' + esc(school_category(row.get("institution", ""))) + '" data-school-type="' + esc(school_type_value) + '" data-subregion="' + esc(subregion or "미지정") + '"' + row_display_attr + '>'
    ...
    for row in records:
        ...
        dashboard_records.append({"id": row.get("id") or row.get("tender_no", ""), "counterpart": row.get("counterpart", "") or "미지정", "amount": amount_number(row.get("amount", "")), "contractDate": row.get("contract_date", ""), "region": record_region or "미지정", "subregion": subregion or "미지정", "supportOffice": record_support or "미지정", "supportByRegion": support_by_region, "subregionByRegion": subregion_by_region, "schoolCategory": school_category(row.get("institution", "")), "schoolType": row.get("school_type", "") or school_type(row.get("institution", ""))})
...
function filterCurrent(keepPage){if(!keepPage){resetPage();}updateUnsavedUi();updateDashboardFilterNote();var rows=document.querySelectorAll('#tbody tr');var matches=[];var purchaseStats=createPurchaseStats();rows.forEach(function(row){var keywords=row.getAttribute('data-keywords')||'';var keywordMatch=activeKeyword==='all'||(','+keywords+',').indexOf(','+activeKeyword+',')!==-1;var contractText=normalizeSearchText(row.getAttribute('data-search-text')||row.getAttribute('data-contract-name')||'');var contractMatch=!contractSearchText||contractText.indexOf(contractSearchText)!==-1;var deleted=row.getAttribute('data-deleted')==='1';var unsavedMatch=!unsavedOnly||!rowHasRegion(row);var dashboardMatch=rowMatchesDashboardFilter(row);var type=classifyPurchaseType(row.getAttribute('data-contract-name')||'',row.getAttribute('data-record-id')||'');row.setAttribute('data-purchase-type',type.key);var purchaseMatch=!activePurchaseType||type.key===activePurchaseType;var vendorScopeMatch=vendorMatchesScope(row);var matched=keywordMatch&&contractMatch&&!deleted&&unsavedMatch&&rowMatchesDate(row)&&dashboardMatch&&purchaseMatch&&vendorScopeMatch;row.setAttribute('data-filter-match',matched?'1':'0');if(matched){matches.push(row);addPurchaseStats(purchaseStats,row,type,amountFromRow(row));}});renderPurchaseStats(purchaseStats);var total=matches.length;var pages=Math.max(1,Math.ceil(total/pageSize));if(currentPage>pages){currentPage=pages;}var start=(currentPage-1)*pageSize;var end=start+pageSize;rows.forEach(function(row){row.style.display='none';var cb=row.querySelector('.row-check');if(cb){cb.checked=false;}var no=row.querySelector('.row-no');if(no){no.textContent='';}});matches.slice(start,end).forEach(function(row,index){row.style.display='';var no=row.querySelector('.row-no');if(no){no.textContent=start+index+1;}});document.getElementById('visible-count').textContent=total;document.getElementById('no-result').style.display=total===0?'block':'none';renderPagination(total);var master=document.querySelector('thead input[type="checkbox"]');if(master){master.checked=false;}}
...
document.addEventListener('DOMContentLoaded',function(){syncDateToggles();applyRegions(getRegionOverrides());applySchoolTypes(getSchoolTypeOverrides());applyDeleted(getDeletedRecords());loadRemoteState();});
""".strip()
