...
document.addEventListener('DOMContentLoaded',function(){var cfg=getSupabaseConfig();var urlInput=document.getElementById('supabase-url');var keyInput=document.getElementById('supabase-key');if(urlInput&&cfg.url){urlInput.placeholder=cfg.url;}if(keyInput&&cfg.key){keyInput.placeholder='anon key 저장됨';}applyRegions(getRegionOverrides());applyDeleted(getDeletedRecords());loadRemoteState();});
""".strip()
    ...
    return (
        ...
        "<div class='sync-panel'><input id='supabase-url' type='text' autocomplete='off' placeholder='Supabase project URL'>"
        "<input id='supabase-key' type='password' autocomplete='off' placeholder='Supabase anon public key'>"
        "<button type='button' class='sync-btn' onclick='saveSupabaseConfig()'>Supabase 저장</button>"
        "<button type='button' class='sync-btn secondary' onclick='clearSupabaseConfig()'>설정 삭제</button>"
        "<span id='sync-status' class='sync-status'>Supabase 설정 후 지역 저장/삭제를 사용할 수 있습니다.</span></div>"
