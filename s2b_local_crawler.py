...
function updateSupportOffice(row,region){if(!row){return;}var cell=row.querySelector('.support-cell');if(!cell){return;}var map={};try{map=JSON.parse(cell.getAttribute('data-support-by-region')||'{}');}catch(e){map={};}var original=cell.getAttribute('data-original-support')||'';cell.textContent=original||((region&&map[region])?map[region]:'');}
...
function dashboardRows(){var regions=getRegionOverrides();var deleted=new Set(getDeletedRecords()||[]);return dashboardRecords.filter(function(row){return !deleted.has(row.id);}).map(function(row){var region=regions[row.id]||row.region||'\uBBF8\uC9C0\uC815';var original=row.supportOffice||'';var support=(original&&original!=='\uBBF8\uC9C0\uC815')?original:((region&&row.supportByRegion&&row.supportByRegion[region])?row.supportByRegion[region]:'\uBBF8\uC9C0\uC815');return Object.assign({},row,{region:region,supportOffice:support});});}
...
function rowSupportValue(row){var region=rowRegionValue(row);var record=dashboardRecordForRow(row);if(record){if(record.supportOffice&&record.supportOffice!=='\uBBF8\uC9C0\uC815'){return record.supportOffice;}if(region&&record.supportByRegion&&record.supportByRegion[region]){return record.supportByRegion[region];}}var cell=row.querySelector('.support-cell');var value=cell?(cell.textContent||'').trim():'';return value||'\uBBF8\uC9C0\uC815';}
